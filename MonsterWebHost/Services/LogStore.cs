...
public sealed class LogStore
{
    ...
    {
        var line = JsonSerializer.Serialize(trafficEvent);
    ...
    public async Task ExportJsonAsync(string outputPath, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var files = Directory.EnumerateFiles(_logDirectory, "requests-*.jsonl")
            ...
            foreach (var file in files)
            {
                ...
                payloads.AddRange(lines.Where(line => !string.IsNullOrWhiteSpace(line)));
            }

            var json = "[" + string.Join(",", payloads) + "]";
